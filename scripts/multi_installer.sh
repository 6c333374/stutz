...
# Stutz Multi-installer
# a one line clone-and-compile for stutz:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/maze2k1/stutz/master/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/stutz ]; then
            read -r -p "${1:-stutz directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old stutz directory" && echo
                    rm -rf "$PWD"/stutz
                    ;;
                *)
                    _fail "stutz directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir stutz
        git clone -q https://github.com/maze2k1/stutz stutz   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd stutz
    fi
}

_build_stutz() {
    _note "Building stutz from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/maze2k1/stutz')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/maze2k1/stutz')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Stutz Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'stutz/build/src/' for the executible binaries. See 'https://github.com/maze2k1/stutz' for more project support. Cowabunga!"
